...
对同一张图片和同一段描述，应尽量保持可见事实、隐患等级和整改方向稳定一致。`
  ...
  async processHazardImagesInspection(params) {
    ...
    const knowledgeKeywords = extractKnowledgeSearchKeywords({ prompt, enterprise, images })
    const matchedKnowledgeClauses = await knowledgeClauseDal.searchActiveByKeywords(knowledgeKeywords, 8)
    const knowledgeRefs = mapKnowledgeRefsForClient(matchedKnowledgeClauses)
    const knowledgeClauseContext = formatKnowledgeClauseContext(matchedKnowledgeClauses)
...
如果【本地知识库命中条款】中存在条款，你只能从这些条款中选择和图片隐患直接相关的依据写入 basis 和 reference_standards；不得引用未提供的具体法规、标准编号或条文内容。如果没有命中条款，法规依据必须保守处理：不能确定时，basis 必须写“需人工复核具体条款”，reference_standards 返回 []。不要为了报告完整性随机选择条款，不要编造不存在的法规、标准编号或条文内容。
...
5) basis 与 reference_standards 必须对应图片隐患，写明规范名称、标准编号/文件号、条款号、具体条款内容；不要只写法规名称，不要沿用模板固定清单，不要编造不存在的法规或条款
   ...
   - 如果本地知识库没有命中相关条款，或命中条款与图片隐患不直接相关，basis 写“需人工复核具体条款”，reference_standards 返回 []，不要输出看似精确但未核验的条款
...
7) 报告整体风格要求：严谨、专业、客观`
    ...
    try {
      ...
      return { result, sessionId, knowledgeRefs }
    } catch (apiError) {
      console.error('[AIService] API error:', apiError)
