...
管理员上传 PDF 或 DOCX 知识文档后，系统会尝试抽取条款，但抽取结果只进入“抽取草稿”池，不会直接进入正式法规条文库。

- PDF/DOCX：生成待审核草稿，管理员可编辑、通过或驳回。
- DOC：旧二进制格式不自动抽取，仅保留文件级管理。
- CSV：作为正式条文入库通道，导入前会执行质量校验。
- 覆盖率看板只统计正式 `knowledge_clauses`，不会把未审核草稿算作可用法规依据。
## 6. 后端启动
