...
- `high`：命中本地启用规则，且规则关联已校验法规条文。
- `medium`：未命中正式规则，但命中本地正式法规条文。
- `low`：本地规则与本地条文都不足，只能给出 AI 低可信参考。
- `non_business`：图片不属于安全生产检查场景。

### 当前项目中的“RAG”定义

本阶段不引入向量数据库或框架化 RAG。当前兜底检索仅指：

- 基于 AI 抽取的可见事实、建议关键词、用户补充描述、企业行业信息；
- 使用 MySQL LIKE / 关键词检索本地 `knowledge_clauses`；
- 将命中的正式条文作为中可信参考，而不是让模型自由编造法规依据。

### 报告快照字段

`inspection_reports` 已保存以下可信度快照字段：
...
这样历史报告不会因后续规则库或法规库变化而丢失当时的依据说明。
...
3. 可选择上传 PDF/Word 文件，或按 CSV 模板批量导入法规条文
4. 文件上传路径：后端接收文档存入 uploads/
5. 后端解析文档：PDF 使用 pdf-parse，DOCX 使用 jszip 读取 word/document.xml，DOC 旧二进制格式仅保留文件级管理
6. CSV 导入路径：后端校验分类、法规名称、文号/标准号、条款号、条文内容、官方来源 URL、发布机关、施行日期和现行状态
7. 识别条款格式或读取 CSV 行 → 拆分/追加条款存入 knowledge_clauses 表，并同步保存官方来源快照
8. 无法识别明确条款编号时 → 按段落降级入库；无法抽取文本时保留知识文档并标记为待人工复核
9. 管理员可查看条款数量、解析状态、来源信息和校验状态，可编辑/归档/批量操作知识文档
10. AI 隐患分析会基于用户描述、企业信息、图片标签和文件名检索本地条款；命中条款只作为受控上下文提供给模型，报告保存后同步保存引用快照
```
...
- 管理端知识库页面按固定 14 类展示法规文件数、结构化条文数、已校验条文数、待校验条文数和不采用条文数。
- 顶部汇总采用全库唯一文档和唯一条文计数；分类卡片采用适用分类计数，因此通用法规关联多个分类时会在多个分类中体现。
- 分类覆盖状态分为：`empty` 空、`unverified` 有条文但未校验、`usable` 已有人工校验条文。
- 知识库为空时，系统前端必须提示“只能进行图片事实识别，不能出具法规判断”。
- 只有存在已人工校验条文时，才可作为后续规则判定和报告依据追溯的基础。
- 规则库模块已新增 `hazard_rules` 和 `hazard_rule_clause_refs`，覆盖率仍聚焦法规文档和正式条文；规则数量在规则库页面单独展示。

### AI 规则草稿设计

- `hazard_rule_drafts` 保存 AI 根据已校验法规条文生成的候选规则，字段与正式规则保持一致，并额外记录 `clause_ids_json`、生成提示词、模型原始返回、生成管理员、审核状态、审核备注和通过后的正式规则 ID；该表随后端 schema 初始化创建，草稿池为空时返回空列表而不是系统错误。
- 草稿只能引用 `knowledge_clauses.verification_status = verified` 且 `current_status` 为 `现行有效` 或 `现行` 的条文；后端会在生成、编辑、通过时重复校验。
- 对总则、目的、适用范围、术语等原则性条文，系统会要求管理员改选具体检查要求、禁止行为、技术指标或判定条款，避免 AI 被迫生成无法落地的图片规则。
- 草稿不进入正式判定链。只有管理员审核通过后，系统才调用正式规则服务写入 `hazard_rules` 和 `hazard_rule_clause_refs`。
- 审核通过生成的正式规则默认不启用，管理员仍需在规则库确认后手动启用，避免 AI 草稿直接影响用户报告结论。

### 隐患规则库设计

PR18 新增规则库层，用于把已校验法规条文转化为后续 AI 判定可使用的业务规则。

核心表：

- `hazard_rules`：保存规则名称、所属分类、隐患等级、可见事实关键词、触发条件、所需证据、图片是否可独立判断、证据不足默认结论、整改建议模板、启用状态和归档状态。
- `hazard_rule_clause_refs`：保存规则与正式法规条文的多对多关联，规则只能关联已校验且当前现行的 `knowledge_clauses`。

业务约束：

- 未启用规则可以作为草稿保存，便于管理员逐步整理依据。
- 任何启用规则必须至少关联 1 条已校验正式条文，避免无依据规则进入后续 AI 判定链。
- 重大隐患和疑似重大隐患属于高风险等级，后续报告和人工复核流程中必须重点处理。
- 种子规则导入按 `seed_key` 幂等处理，重复导入会更新既有规则，不重复创建。
- 种子规则依据优先选择 GB 标准、重大隐患判定规则、检查规则等更具体条款；每条种子规则默认只绑定 1-2 条主依据，避免正式报告一次性展开大量宽泛法条。
- 找不到本地已校验依据条文的种子规则会跳过，不会为了演示强行启用。
### 规则驱动 AI 初判流程

PR19 将 `/api/hazard/analyze` 从“模型直接生成隐患结论”调整为“AI 事实抽取 + 后端规则判定”：

1. AI 先判断图片是否属于安全生产检查场景。
2. 无关图片直接返回 `report_allowed = false`，只保存分析记录，不生成正式 Word/PDF。
3. 相关图片只抽取可见事实、证据不足点和建议关键词。
4. 后端读取 `hazard_rules` 中已启用且已关联正式条文的规则。
5. 后端按可见事实关键词匹配规则，生成 `matched_rules`、`legal_refs`、`hazard_level` 和 `evidence_sufficiency`。
6. 未命中规则或证据不足时，结论为“需人工复核”或规则配置的疑似等级，不生成正式报告。
7. 命中启用规则且图片证据充分时，才允许生成正式报告。

该流程不引入向量数据库、RAG 框架或消息队列，仍使用 MySQL 和规则关键词匹配，保持第一阶段工程边界可控。
## 8. 接口规范
...
| 第三批 | 企业数据查询 + 操作日志 + 报告模板 + 数据备份 | 已完成 | 企业、日志、工作台、报告模板、知识库和手动备份均已接入真实后端；自动备份仍标记为规划中 |
...
- **数据备份**：支持真实手动 MySQL 备份、备份记录展示和受控下载；自动备份仅展示规划状态；暂不提供高风险数据库恢复操作。
- **微信端适配**：业务按钮、列表和弹窗均在业务页面直接渲染，避免多层组件插槽导致微信端点击或内容渲染失效。
- **数据状态**：第三阶段中企业数据查询、操作日志、管理员工作台、报告模板、知识库和手动备份已接入真实后端；自动备份仍待后续 PR 落地。
...
本阶段将业务模型调整为安全服务公司内部检查员使用：检查员不是被检查企业员工，`enterprises` 表统一作为被检查客户企业档案。检查员每次选择或新建客户企业后创建 `inspection_tasks`，后续图片、AI 分析记录和报告都通过 `inspection_task_id` 归档。

### 核心表关系
- `enterprises`: 被检查客户企业主档案。
- `inspection_tasks`: 检查任务，一条任务绑定一个客户企业和一个检查员。
- `hazard_images.inspection_task_id`: 图片归属的检查任务。
- `inspection_reports.inspection_task_id`: 报告归属的检查任务。

### 数据隔离原则
- 管理员可查看全部客户企业、检查任务、图片和报告。
- 普通检查员只能查看和操作自己创建的检查任务。
- 上传图片和发起 AI 分析时必须提供检查任务，后端从任务推导客户企业，不再信任客户端传入的 `enterprise_id`。
- 同一次分析中的图片必须属于同一个检查任务和客户企业。

### 兼容说明
- 历史企业字段继续复用，不在本阶段做大规模重命名。
- 本地演示企业、演示账号、旧报告清理属于合并后的本地数据库操作，不进入 Git。
