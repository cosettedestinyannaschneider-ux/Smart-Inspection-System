...
- `hazard_rule_drafts` 保存 AI 根据已校验法规条文生成的候选规则，字段与正式规则保持一致，并额外记录 `clause_ids_json`、生成提示词、模型原始返回、生成管理员、审核状态、审核备注和通过后的正式规则 ID。
- 草稿只能引用 `knowledge_clauses.verification_status = verified` 且 `current_status = 现行有效` 的条文；后端会在生成、编辑、通过时重复校验。
...
- `hazard_rule_clause_refs`：保存规则与正式法规条文的多对多关联，规则只能关联已校验且现行有效的 `knowledge_clauses`。
